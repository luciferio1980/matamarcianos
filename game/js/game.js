...
AR.Game = {
  ...
  updPlay: function (dt) {
    ...
    if (this.tunnel > 0) {
      ...
      if (!pl.dead && (pl.y < wall + 10 || pl.y > AR.H - wall - 10)) AR.Combat.hurtPlayer(pl.x, pl.y);
    }
  ...
  draw: function () {
    ...
    if (st === "title") {
      ...
      if (this.tunnel > 0) {
        ...
        var wall = 90 + Math.sin(this.t * 8) * 20;
        ctx.fillRect(0, 0, AR.W, wall);
        ctx.fillRect(0, AR.H - wall, AR.W, wall);
        ctx.fillStyle = "rgba(255,60,180,0.15)";
        ctx.fillRect(0, wall - 6, AR.W, 6);
        ctx.fillRect(0, AR.H - wall, AR.W, 6);
      }
